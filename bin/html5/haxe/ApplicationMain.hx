...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		var config = {
			...
			name: "NotSpaceWar",
			orientation: "",
			packageName: "com.adrienheisch.notspacewar",
			version: "1.0.0",
			windows: [
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("com.adrienheisch.notspacewar.Main"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess com.adrienheisch.notspacewar.Main.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"com.adrienheisch.notspacewar.Main\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"com.adrienheisch.notspacewar.Main\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends com.adrienheisch.notspacewar.Main {}
